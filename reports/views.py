...
def get_db_for_ctx():
    ...
    sql = "select date_format(convert_tz(max(last_sync), '+00:00', 'system'), '%Y-%m-%d %H:%i:%s') as sync_time from lanrev_last_export_date"
...
def index(request):
    ctx = {"db": get_db_for_ctx(), "reports": reports}
...
def detail(request, report_id):
    ...
    ctx = {"db": get_db_for_ctx, "report": report}
    cur = connection.cursor()
    prompts = list()
    if report.prompts is None:
        ...
        for prompt in report.prompts:
            if prompt in request.GET:
                ...
                ctx = {"db": get_db_for_ctx, "report": report, "missing_prompt": prompt}
                response = render(request, "reports/err_prompt.html", ctx)
                response["Content-Type"] = "text/html; charset=windows-1252"
                return response
        cur.execute(report.sql, prompts)
    data = cur.fetchall()
    ctx = {"db": get_db_for_ctx, "report": report, "data": data, "prompts": prompts}
...
def detail_csv(request, report_id):
    ...
    response["Content-Disposition"] = 'attachment; filename="{0}.csv"'.format(report.name)
